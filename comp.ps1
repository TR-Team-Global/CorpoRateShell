...
filter domain {
    ...
    if($Credential) {
        ...
        Write-Verbose "Using alternate credentials for domain"
...
filter domainsearcher {
    ...
    if(-not $Credential) {
        ...
        elseif(-not $DomainController) {
            try {
                ...
                throw "domainsearcher: Error in retrieving PDC for current domain"
            }
        }
    }
    elseif (-not $DomainController) {
        ...
        try {
            $DomainController = ((domain -Credential $Credential).PdcRoleOwner).Name
        }
        catch {
            throw "domainsearcher: Error in retrieving PDC for current domain"
        }
    ...
    $SearchString += $DN
    Write-Verbose "domainsearcher search string: $SearchString"
...
function Computer {
    ...
    begin {
        ...
        $CompSearcher = domainsearcher -Domain $Domain -DomainController $DomainController -ADSpath $ADSpath -PageSize $PageSize -Credential $Credential
    }
    ...
    process {
        ...
        if ($CompSearcher) {
            ...
            Write-Verbose "Computer filter : '$CompFilter'"
